...
if($title=="" || $autor=="" || $content==""){
	...
	if(mysql_num_rows($sql)){//判断新闻标题冲突
		...
		if($row[2]==$title){//判断是传过来的新闻标题和库查出来的是否一致，一致就可以直接改其他信息
			mysql_query("UPDATE  `school`.`news` SET  `classname` =  '".$classname."',`autor` =  '".$autor."',`content`='".$content."',`thumbnail`='".$lujing."',`updatetime`='".$updatetime."' WHERE  `news`.`title`='".$title."'");//执行mysql语句，修改新闻
			...
			if(isset($_SESSION['id'])){
			$id=$_SESSION["id"];
			mysql_query("UPDATE  `school`.`news` SET  `classname` =  '".$classname."',`autor` =  '".$autor."',`content` =  '".$content."',`title`='".$title."',`thumbnail`='".$lujing."',`updatetime`='".$updatetime."' WHERE  `news`.`id` =".$id."");	
